...
- **⚡ 文件夹重排权重控制升级**：
    - **新增“允许文件夹参与排序”控制开关**：
        - **关闭（默认模式）**：严格锁定文件夹的原始槽位（Index），仅提取槽位中间的普通书签按点击量重排，完全不影响文件夹的位置。
        - **开启（进阶模式）**：递归计算文件夹下所有子孙书签的点击量之和作为该文件夹的综合权重（Visits），将文件夹与普通书签统一按热度降序排列。
    - **国际化（i18n）适配**：全界面中英文无缝切换。
...
#### 🆕 v1.0.1 (Latest)
- **⚡ Folder Sorting Flexibility Control**:
    - **New "Allow folders to participate in sorting" Toggle**:
        - **Disabled (Default)**: Fixes folder slot positions (Index) and only reorders regular bookmarks within their original slots by visit count, leaving folder positions 100% untouched.
        - **Enabled**: Recursively calculates the total visit score of all child bookmarks inside a folder and ranks folders alongside regular bookmarks based on aggregated usage frequency.
    - **Full i18n Localization**: Added complete English and Chinese translation support across options and dialogs.
